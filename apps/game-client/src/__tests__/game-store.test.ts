...
describe('GameStore', () => {
  ...
  it('markFieldsFlash sets flashedFields for 800ms then clears', async () => {
    ...
    expect(useGameStore.getState().flashedFields).toEqual(['inventory', 'flags']);
  ...
  it('only valid actions are present in allowedActions after hydration', () => {
    ...
    expect(state.allowedActions.map((a) => a.template_id)).toContain('action-examine');
    expect(state.allowedActions.map((a) => a.template_id)).toContain('action-access-vault');
  });
});
